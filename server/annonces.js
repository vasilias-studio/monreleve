...
export async function likedIds(userId, ids = []) {
  if (!userId || !ids.length) return new Set();
  const marks = ids.map(() => '?').join(',');
  return new Set((await db.prepare(`SELECT announcement_id FROM announcement_likes WHERE user_id=? AND announcement_id IN (${marks})`).all(userId, ...ids)).map((r) => r.announcement_id));
}

/** « J'aime » : bascule (ajoute ou retire), renvoie true si le like est actif après l'appel. */
export async function toggleLike(announcementId, userId) {
  return await tx(async () => {
    const del = await db.prepare('DELETE FROM announcement_likes WHERE announcement_id=? AND user_id=?').run(announcementId, userId);
    if (del.changes) return false;
    await db.prepare('INSERT INTO announcement_likes (announcement_id, user_id) VALUES (?,?)').run(announcementId, userId);
    return true;
  });
}
 ...
 */
export async function ensureAnnonces() {
  if ((await db.prepare('SELECT COUNT(*) n FROM announcements').get()).n > 0) return 0;
  ...
  return await tx(async () => {
    const ins = await db.prepare("INSERT INTO announcements (author_id, body, audience, program_id, class_id, pinned, created_at) VALUES (?,?,?,?,?,?,datetime('now', ?))");
    let n = 0;
    for (const d of demo) {
