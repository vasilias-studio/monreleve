...
async function studentPayload(student) {
  const u = await db.prepare('SELECT * FROM users WHERE id=?').get(student.user_id);
  const names = async (sql, id) => (id ? (await db.prepare(sql).get(id))?.name ?? null : null);
  return {
    ...publicUser(u),
    student: {
      id: student.id,
      matricule: student.matricule,
      program: await names('SELECT name FROM programs WHERE id=?', student.program_id),
...
r.post('/register', async (req, res, next) => {
  try {
    ...
    const student = await db.prepare('SELECT * FROM students WHERE id=?').get(created.studentId);
    res.status(201).json({ token: signToken(user), ...await studentPayload(student) });
  } catch (e) { next(e); }
});
...
r.post('/login', async (req, res, next) => {
  try {
    ...
    if (user.role === 'student') {
      const student = await db.prepare('SELECT * FROM students WHERE user_id=?').get(user.id);
      base.student = (await studentPayload(student)).student;
    }
    res.json(base);
  } catch (e) { next(e); }
});
...
r.get('/me', authRequired, async (req, res, next) => {
  try {
    if (req.user.role === 'student') {
      ...
      return res.json(await studentPayload(student));
    }
...
r.put('/me', authRequired, async (req, res, next) => {
  try {
    const b = req.body || {};
    ...
    await db.prepare(`UPDATE users SET ${Object.keys(updates).map((k) => k + updates[k]).join(',')} WHERE id=?`).run(...params);
    res.json(publicUser(await db.prepare('SELECT * FROM users WHERE id=?').get(req.user.id)));
  } catch (e) { next(e); }
});
...
r.post('/reset', async (req, res, next) => {
  try {
    ...
    const user = await db.prepare(`SELECT * FROM users WHERE reset_token=? AND reset_expires > datetime('now')`).get(String(token));
    if (!user) throw badRequest('Lien invalide ou expiré');
    await db.prepare(`UPDATE users SET password_hash=?, reset_token=NULL, reset_expires=NULL WHERE id=?`).run(bcrypt.hashSync(String(password), 10), user.id);
    res.json({ ok: true });
  } catch (e) { next(e); }
});
