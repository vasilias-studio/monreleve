...
export async function findTemplateForStudent(student) {
  if (!student.program_id || !student.level_id) return null;
  const yearId = student.academic_year_id ?? (await db.prepare('SELECT id FROM academic_years WHERE is_current=1').get())?.id ?? null;
  const t = await db.prepare(`SELECT t.*, p.name AS program_name, l.name AS level_name, y.label AS year_label
...
export async function loadTemplateTree(templateId) {
  const sems = await db.prepare('SELECT * FROM semesters WHERE template_id=? ORDER BY ord, number').all(templateId);
  const units = await db.prepare('SELECT * FROM units WHERE semester_id=? ORDER BY ord, id');
  const courses = await db.prepare('SELECT * FROM courses WHERE unit_id=? ORDER BY ord, id');
  for (const s of sems) {
    s.units = await units.all(s.id);
    for (const u of s.units) u.courses = await courses.all(u.id);
  }
  return sems;
}

/** Charge les notes d'un étudiant pour un modèle et une source. */
async function loadGrades(studentId, semesters, source) {
  const ids = semesters.flatMap((s) => s.units.flatMap((u) => u.courses.map((c) => c.id)));
  const map = new Map();
  if (!ids.length) return map;
  const marks = ids.map(() => '?').join(',');
  const rows = await db.prepare(`SELECT course_id, normal, rattrapage FROM grades WHERE student_id=? AND source=? AND course_id IN (${marks})`)
    .all(studentId, source, ...ids);
  for (const row of rows) map.set(row.course_id, row);
  return map;
}

async function publicationMap(semesters) {
  const out = {};
  for (const s of semesters) {
    out[s.id] = await db.prepare('SELECT status, published_at FROM publications WHERE semester_id=?').get(s.id) || { status: 'draft', published_at: null };
  }
  return out;
}
...
r.get('/overview', async (req, res, next) => {
  try {
    const template = await findTemplateForStudent(req.student);
    if (!template) return res.json({ template: null });
    const semesters = await loadTemplateTree(template.id);
    const personal = computeReleve(template, semesters, await loadGrades(req.student.id, semesters, 'personal'));
    const official = computeReleve(template, semesters, await loadGrades(req.student.id, semesters, 'official'));
    const pubs = await publicationMap(semesters);
...
r.get('/releve', async (req, res, next) => {
  try {
    ...
    const semesters = await loadTemplateTree(template.id);
    const grades = await loadGrades(req.student.id, semesters, source);
    ...
    if (source === 'official') {
      ...
      const officialTotals = computeReleve(template, visibleTree, await loadGrades(req.student.id, visibleTree, 'official'));
...
r.put('/grades/:courseId', async (req, res, next) => {
  try {
    const courseId = Number(req.params.courseId);
    const course = await db.prepare(`SELECT c.*, u.semester_id, s2.number AS sem_number
    ...
    const inTemplate = await db.prepare('SELECT 1 FROM semesters WHERE template_id=? AND id=?').get(template.id, course.semester_id);
    ...
    const semesters = await loadTemplateTree(template.id);
    const computed = computeReleve(template, semesters, await loadGrades(req.student.id, semesters, 'personal'));
...
r.put('/grades', async (req, res, next) => {
  try {
    const template = await findTemplateForStudent(req.student);
    if (!template) throw forbidden('Aucun modèle associé à votre compte');
    const allowed = new Set((await loadTemplateTree(template.id)).flatMap((s) => s.units.flatMap((u) => u.courses.map((c) => c.id))));
...
r.get('/stats', async (req, res, next) => {
  try {
    ...
    const semesters = await loadTemplateTree(template.id);
    const mk = async (src) => {
      const c = computeReleve(template, semesters, await loadGrades(req.student.id, semesters, src));
      return {
    ...
    res.json({ personal: await mk('personal'), official: await mk('official') });
  } catch (e) { next(e); }
});
