...
export function page(ctx, { title = 'MonRelevé', body = '', tabs = null, adminTab = null, bare = false } = {}) {
  ...
  /* Pastille (repère Frame-101 v2) : 4 emplacements fixes ; le disque crème glisse sous l'onglet courant,
     l'icône active passe au cuivre. Sans JavaScript le rendu reste correct (position servie par le serveur). */
  const PILL_OF = { home: 'home', pencil: 'pencil', list: 'chart', chart: 'chart', calendar: 'calendar' };
  ...
  const adminNav = adminTab ? `<nav style="position:sticky;top:55px;z-index:25;background:color-mix(in srgb, var(--card) 92%, transparent);backdrop-filter:blur(10px);border-bottom:0;padding:8px 12px;display:flex;gap:6px;overflow-x:auto">
      ${[['/accueil', 'Annonces'], ['/admin', 'Tableau de bord'], ['/admin/etudiants', 'Étudiants'], ['/admin/modeles', 'Modèles'], ['/admin/import', 'Import Excel'], ['/admin/referentiels', 'Référentiels'], ['/admin/emploi', 'Calendrier']]
